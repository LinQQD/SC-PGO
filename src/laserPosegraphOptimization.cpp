...
void reportScPgoIdleIfReady()
{
    ...
    if (no_recent_input && no_recent_work && buffers_drained && loop_drained && graph_drained && map_refreshed) {
        RCLCPP_WARN(g_node->get_logger(), "[SC-PGO] idle: no new odom/cloud for %.1f s, loop/ICP queue drained, graph optimized. Safe to call: ros2 service call /save_map std_srvs/srv/Empty. remaining_odom=%zu remaining_cloud=%zu remaining_icp=%zu",
